...
class ValidationMixin:
    ...
    def validate_nombre(self, nombre, campo="Nombre"):
        ...
        # Dividir el nombre en caso de que sea compuesto y capitalizar cada parte
...
class Command(ValidationMixin, createsuperuser.Command):
    help = 'Crea un usuario regular y su empleado asociado con validaciones'
        ...
        with transaction.atomic():
            try:
                ...
                self.stdout.write(self.style.SUCCESS(
                    f"Usuario regular y Empleado {empleado.nombre} {empleado.apellido} creados exitosamente."
                ))
            except Exception as e:
                self.stderr.write(self.style.ERROR(f"Error al crear usuario regular y empleado: {str(e)}"))
                raise CommandError(f"Error al crear usuario regular y empleado: {str(e)}")